...
**When:** Every 3 hours (8 times per day)
...
- Changes appear in search within <time_range> hours of being made
...
**Technical Note:**  
The extension does not implement webhooks in version 1.3.1. Luigi's Box uses a polling mechanism (periodic API calls) to detect changes. The polling schedule is configured in Luigi's Box Web Application under **Catalog → Settings → Feeds Planning** and defaults to 8 syncs per day (every 3 hours). This schedule can be adjusted in the Luigi's Box dashboard if needed for your specific use case.
...
**Limitation:** Manual sync can only be triggered **once every 24 hours**
...
**Note:** If you need more frequent updates, adjust the automatic sync schedule instead of relying on manual sync.
...
- Last sync more than 3 hours ago
...
- Luigi's Box detects new products on next sync cycle (every 3 hours)
...
**Typical Timing:** Changes appear within 3 hours (based on default sync schedule)

**To Speed Up:**

- Trigger manual sync using **Run Feed Now** button (once per 24 hours max)
- Or increase sync frequency in **Catalog → Settings → Feeds Planning**
...
- On next sync cycle (within 3 hours): Price changes detected
...
- On next sync cycle (within 3 hours): Inventory change detected
...
- On next sync cycle (within 3 hours): Deletion detected
...
**Automatic Sync:** Every 3 hours (8 times per day)
...
**Configuration:** The sync schedule is configured in Luigi's Box Web Application under **Catalog → Settings → Feeds Planning**. The default is 8 syncs per day (every 3 hours), but you can adjust this frequency based on your needs:
...
**Remember:** Manual sync (**Run Feed Now**) can only be used once every 24 hours. For more frequent updates, adjust the automatic sync schedule.

**When NOT to Trigger:**

- After single product updates (wait for auto-sync)
- Multiple times in succession (not possible - 24 hour limit)
...
- Trigger manual sync (if not used in past 24 hours)
...
- Trigger manual sync (if available - once per 24 hours)
