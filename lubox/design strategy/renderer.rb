...
class Renderer < Middleman::Renderers::MiddlemanRedcarpetHTML
  ...
  def preprocess(text, options = {})
    # First, handle block-style helpers
  ...
  def api_section(prose_content, code_examples, code_title = nil, options = {})
    code_section_html = if code_title && !code_title.empty?
      %Q{
  ...
  # Uses Redcarpet directly with syntax highlighting
  def markdown(content)
    # Don't use our custom render() to avoid infinite recursion
    # Instead, create a new markdown engine with a fresh Renderer instance
    nested_renderer = self.class.new
    markdown_engine = Redcarpet::Markdown.new(nested_renderer, {
    ...
    # Call Redcarpet's render directly (not our overridden version)
    # This will use block_code for syntax highlighting but skip preprocessing
    html = markdown_engine.render(content.to_s)
    
    # Apply postprocessing for code block tabs
