document.addEventListener("DOMContentLoaded", function () {
  ...
  async function getPLPResults(page = 1, filters = {}) {
    ...
    try {
      ...
      productGrid.innerHTML =
        "<p>Sorry, there was an error loading products.</p>";
    }
  }
  ...
  function renderProducts(hits = [], total_hits = 0) {
    ...
    productGrid.innerHTML = hits
      .map((product) => {
        const imageUrl =
          product.attributes.image_link ||
          `https://placehold.co/300x200/eee/ccc?text=No+Image`;
        const productId =
          product.attributes.id && product.attributes.id[0]
            ? product.attributes.id[0]
            : null;
        const productIdAttribute = productId
        ...
        return `
                <div class="product-card">
                    <a href="${
                      product.url
                    }" class="product-link" ${productIdAttribute} target="_blank">
                        <img src="${imageUrl}" alt="${
          product.attributes.title
        }" onerror="this.onerror=null;this.src='https://placehold.co/300x200/eee/ccc?text=No+Image';">
                    </a>
    ...
    facetsContainer.innerHTML = facets
      .map((facet) => {
        const values = facet.values
          .map((val) => {
            const isChecked = (activeFilters[facet.name] || []).includes(
              val.value
            );
            return `
                    <li>
  ...
  function renderPagination(totalHits) {
    ...
    let paginationHTML = "";
    paginationHTML += `<button class="pagination-button" data-page="1" ${
